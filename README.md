...
1. `Management.war`和`Examclient.war`需要部署在`同一个目录`下。
...
1. 将`Management.war`和`Examclient.war`放到Tomcat应用程序目录（`webapps`）下。
	
1. 启动Tomcat，webapps目录下会生成两个文件夹（`Management`和`Examclient`）。
	
1. 分别进入到`Management/WEB-INF/Spring`和`Examclient/WEB-INF/Spring`下修改`root-context.xml`文件,将数据库地址、用户名和密码修改成正确的内容。修改完成后重启tomcat服务器。
...
1. 访问`http://localhost:8080/Management`和`http://localhost:8080/Examclient`可以进入到管理后台页面和学员页面，并可以正常登录，则应用配置成功。
