...
public class ExamPageTeacher {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/teacher/exam/student-exampaper/{examId}/{examhistoryId}", method = RequestMethod.GET)
	private String markExampaperInnerPage(Model model, HttpServletRequest request, @PathVariable int examId, @PathVariable int examhistoryId,@RequestParam(value="searchStr",required=false,defaultValue="") String searchStr,@RequestParam(value="page",required=false,defaultValue="1") int page) {
		...
		List<ExamHistory> histList = examService.getUserExampaperHistListByExamId(examId, searchStr, pageModel);
		String pageStr = PagingUtil.getPagelink(page,
				pageModel.getTotalPage(),"","admin/exam/student-exampaper/" + examId+"/0");

		if(histList.size()>0){
			if(examhistoryId==0){
				examhistoryId=histList.get(0).getHistId();
			}
			ExamHistory history = examService.getUserExamHistListByHistId(examhistoryId);
			int examPaperId = history.getExamPaperId();
			...
			model.addAttribute("examHistoryId", history.getHistId());
			model.addAttribute("exampapername", examPaper.getName());
			model.addAttribute("studentName", history.getTrueName());
		}
		model.addAttribute("histList", histList);
		model.addAttribute("pageStr", pageStr);
		model.addAttribute("examId", examId);
		model.addAttribute("searchStr", searchStr);
		return "student-exampaper-list";
	}
}
