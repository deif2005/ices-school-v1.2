...
import cn.mteach.common.jyeoo.api.QuestionApiService;
import cn.mteach.common.util.Page;
import cn.mteach.common.util.ehcache.GetCacheData;
import cn.mteach.common.util.file.ExcelUtil;
...
public class QuestionServiceImpl implements QuestionService {
	...
	public Boolean isExistSubject(String fieldName){
		List list=questionMapper.searchFieldByName(fieldName);
		if(list.size()>0){
			return true;
		}else {
			return false;
...
//				+ filePath;
	...
	public void addReference(Reference reference){
		questionMapper.addReference(reference);
	}
	@Override
	public Boolean isExistReference(String referenceName,String subjectName){
		List<Reference> list=questionMapper.searchReferenceByName(referenceName,subjectName);
		if(list.size()>0){
			return true;
		}else{
			return false;
	...
	public List<Chapter> getChapter(Chapter chapter){
		...
		List<ChapterDetail> chapterDetails = null;
		chapterList = questionMapper.getChapter(chapter);
		try {
			chapters.clear();
			if (chapterList.size() == 0){
				//获取远程数据
				String jsArray = BaseInfoApiService.getChapterInfo(GetCacheData.getJyeooToken(),chapter);
				chapterDetails = parseJsonArrayWithGson(jsArray,ChapterDetail.class);
				if (chapterDetails.size() > 0 ){
					for (int i=0; i<chapterDetails.size(); i++){
						ChapterDetail chapterDetail = chapterDetails.get(i);
						if (chapterDetails.get(i).getChildren().size() > 0){
							getChapterInfo(chapterDetail,chapterDetails.get(i).getChildren());
						}
					}
					questionMapper.addChapter(chapters);
	...
	// 将Json数组解析成相应的映射对象列表
	public static <T> List<T> parseJsonArrayWithGson(String jsonData, Class<ChapterDetail> type) {
		Gson gson = new Gson();
		List<T> result = gson.fromJson(jsonData, new TypeToken<List<ChapterDetail>>() {}.getType());
		return result;
	}
	 ...
	 * 递归解析章节信息
	 */
	private void getChapterInfo(ChapterDetail chapterDetail,List<ChapterDetail.Children> childrens){
		if (childrens.size() > 0){
			for (int i=0; i<childrens.size(); i++){
				ChapterDetail.Children children = childrens.get(i);
				Chapter chapter = new Chapter();
				chapter.setChapterId(children.getID());
				chapter.setParentId(children.getPID());
				chapter.setChapterName(children.getName());
				chapter.setSubject(chapterDetail.getSubject());
				chapter.setGradeId(chapterDetail.getGradeID());
				chapter.setTermId(chapterDetail.getTermID());
				chapter.setEditionId(chapterDetail.getEditionID());
				List<ChapterDetail.Points> pointses = children.getPoints();
				for (int n=0; n<pointses.size();n++){
					KnowledgePoint point = new KnowledgePoint();
					point.setPointName(pointses.get(n).getValue());
					point.setChapterId(children.getID());
					Field field=jyeooMapper.getFieldByKeyId(chapterDetail.getSubject());
					point.setFieldId(field.getFieldId());
					questionMapper.addKnowledgePoint(point);
				}
				chapter.setPoints(pointses);
				chapters.add(chapter);
				if (children.getChildren().size() > 0){
					getChapterInfo(chapterDetail,children.getChildren());
				}
			}
		}
	}
	 ...
	 * 获取题目信息
	 */
	public List<Question> getRemortQuestion(QuestionParam questionParam){
		StringBuilder stringBuilder = new StringBuilder();
		List<JyQuestion> jyQuestions = new ArrayList<>();
		Gson gson = new Gson();
		//获取远程数据
		try {
			String js = QuestionApiService.SearchQues(GetCacheData.getJyeooToken(),questionParam.getSubject(),
					questionParam.getTp(),questionParam.getP1(),questionParam.getP2(),questionParam.getP3(),
					questionParam.getDg(),questionParam.getCt(),questionParam.getSc(),questionParam.getGc(),
					questionParam.getRc(),questionParam.getYc(),questionParam.getEc(),questionParam.getEr(),
					questionParam.getPo(),questionParam.getPd(),questionParam.getPi(),questionParam.getPs());

			QuestionDetail questionDetail = parseJsonWithGson(js,QuestionDetail.class);
			if (questionDetail.getTotalPage() > 0) {
				List<QuestionDetail.QuestionData> datas = questionDetail.getData();
				for (int i=0; i<datas.size(); i++){
					JyQuestion jyQuestion = new JyQuestion();
					jyQuestion.setId(datas.get(i).getID());
					jyQuestion.setSubject(datas.get(i).getSubject());
					jyQuestion.setCate(datas.get(i).getCate());
					jyQuestion.setLabel(datas.get(i).getLabel());
					jyQuestion.setContent(datas.get(i).getContent());
					ArrayList<String> list = (ArrayList<String>)datas.get(i).getOptions();
					jyQuestion.setOptions(list.toString());
					ArrayList<String> list1 = (ArrayList<String>)datas.get(i).getAnswers();
					jyQuestion.setAnswer(list1.toString());
					ArrayList<String> list2 = (ArrayList<String>)datas.get(i).getQuesFiles();
					jyQuestion.setAnswer(list2.toString());
					jyQuestion.setPOINT(datas.get(i).getPoint());
					jyQuestion.setDegree(datas.get(i).getDegree());
					jyQuestion.setPapertime(datas.get(i).getPaperTime());
					jyQuestion.setQuestionJson(gson.toJson(datas));
					jyQuestions.add(jyQuestion);
				}
				questionMapper.addQuestionDetail(jyQuestions);
			}
		}catch (Exception e){
			e.printStackTrace();
			throw new RuntimeException(e);
