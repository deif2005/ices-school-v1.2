...
$(function() {
	ueditorInit();
	//textObj["textarea63"].setContent('aaaaaaaaaaaaa');
	//var a={};
	//a.test1=UE.getEditor('textarea63');
	////test1.setContent('11111');
	//a.test1.addListener("ready", function () {
	//	a.test1.setContent('aaaa');
	//});
	...
	//初始化状态值
	//var thisAddListener=function(thisKey){
	//		var current_index=$("li.question").index($('#'+thisKey).parent().parent());
	//		//console.log(current_index)
	//		if(textObj[thisKey].getContent() != ''){
...
function ueditorInit(){
	...
	for(var i=0;i<textarea.length;i++){
		if(textarea.eq(i).attr('id')){
			textId=textarea.eq(i).attr('id');
			textObj[textId]=UE.getEditor(textId,{toolbars: [['pasteplain','fullscreen', 'undo', 'redo', 'insertimage','imagefloat','subscript','superscript','preview','print']]});
...
//		this.securityHandler();
	...
	bindFinishOne : function bindFinishOne() {
		...
		//TODO 因百度富文本bug 所以写成new新的对象进行绑定
		var thisAddListener=function(thisKey){
			textObj[thisKey].addListener( "selectionchange", function () {
				var current_index=$("li.question").index($('#'+thisKey).parent().parent());
				//console.log(current_index)
				if(textObj[thisKey].getContent() != ''){
					$($("a.question-navi-item")[current_index]).addClass("pressed");
		...
		request.done(function(message, tst, jqXHR) {
			...
			if (message.result == "success") {
				...
				util.success("交卷成功！", function() {
					window.location.replace(document.getElementsByTagName('base')[0].href + 'student/exam/finished-submit');
	...
	genrateAnswerSheet : function genrateAnswerSheet() {
		...
		for (var i = 0; i < questions.length; i++) {
			...
			if ($(questions[i]).hasClass("qt-singlechoice")) {
				...
				answerSheetItem.answer = textObj[$(questions[i]).find('.question-textarea').attr('id')].getContent();
...
//		var timestamp = parseInt($("#exam-timestamp").text());
		...
		var textInit=function(id,value){
			console.log(id)
			console.log(value)
			textObj[id].addListener("ready", function () {
				textObj[id].setContent(value);
			});
		}
		...
		for(var i = 0 ; i < list.length; i++){
			if(list[i].questionTypeId == 1){
				...
				var tmp_answer = list[i].answer;
				if(tmp_answer == "") continue;
				new textInit($(questions[i]).find('.question-textarea').attr('id'),tmp_answer);
				$(quetion_navis[i]).addClass("pressed");
