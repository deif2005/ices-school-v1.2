...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = '../../../../secure/upload-question-img',
                    ...
                    if (browser.ie && browser.version <= 7) {
                        ...
                        uploader.makeThumb(file, function (error, src) {
                            if (error || !src) {
                                ...
                                var $img = $('<img src="' + src + '" class="chad-test">');
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                data.fileType = file.file.ext;
                //TODO  设置代码在服务端
                data.examTimeId = sessionStorage.imageExamTimeId;
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    if (ret) {
                        _this.imageList.push({src:ret[0],title:ret[1]});
                        $file.append('<span class="success"></span>');
                    } else {
                        //$file.find('.error').text(json.state).show();
                        $file.find('.error').text("上传图片失败").show();
                    }
                } catch (e) {
...
function urlAddData(url){
    var tempUrl=url.substring('?')[0];
    var data=url.substring('?')[1];

    if(sessionStorage.imageExamTimeId || sessionStorage.imageExamTimeId == '0'){
        tempUrl+='/0';
    }else{
