...
public class PracticePage {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/student/practice-list/practice-improve/{fieldId}/{knowledgePointId}/{questionTypeId}", method = RequestMethod.GET)
	public String practiceImprove(Model model, HttpServletRequest request,
		...
		//TODO 因业务需求 添加js代码 保证上传图片得到examTimeId
		sb.append("<script>sessionStorage.imageExamTimeId='practice'</script>");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/student/practice-list/practice-incorrect/{fieldId}/{knowledgePointId}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/student/practice-list/practice-test/{fieldId}", method = RequestMethod.GET)
	public String practiceStartNew(Model model, HttpServletRequest request,@PathVariable("fieldId") int fieldId) {

		String strUrl = "http://" + request.getServerName() // 服务器地址
				+ ":" + request.getServerPort() + "/";
		
		Map<Integer, Map<Integer, List<QuestionQueryResult>>>  map =
		...
		for(Map.Entry<Integer, Map<Integer, List<QuestionQueryResult>>> entry : map.entrySet()){
			...
			if (entry.getValue().containsKey(4))
				qqrList.addAll(entry.getValue().get(4));
			if (entry.getValue().containsKey(5))
				qqrList.addAll(entry.getValue().get(5));
			if (entry.getValue().containsKey(6))
				qqrList.addAll(entry.getValue().get(6));
			if(entry.getValue().containsKey(7))
				qqrList.addAll(entry.getValue().get(7));
		}
		int amount = 0;
		if(qqrList.size() == 0){
