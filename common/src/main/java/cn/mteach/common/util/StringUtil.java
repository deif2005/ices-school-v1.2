 ...
 *      Author: blueeagle
 */
	public static String chanageExtensionName(String filename,String extName) {
		String fileNameNoExtension = getFileNameNoExtensionName(filename);
		String fileExtName = extName.startsWith(".")?extName:"."+extName;
		return fileNameNoExtension+fileExtName;
	}
	 ...
	 * json串解析
	 */
	public static void decodeJsonString(String jsonStr) {
...
//		JSONObject jobj=JSONObject.fromObject(jsonStr,jc);
		JSONArray jsonArray = JSONArray.fromObject(jsonStr,jc);
		//递归遍历
		decodeJSONArray(jsonArray);
	}

	public static void decodeJSONArray(JSONArray json){
		Object o;
		JSONObject jo = null;
		JSONArray ja = null;
		for (int i=0;i<json.size();i++){
			...
			if (o instanceof JSONObject){
				...
				for (int j=0;j<ja.size();j++){
					decodeJSONObject(ja.getJSONObject(j));
				}
			}else{
